...
class GifImage
{
    ...
    {
        $this->img_effect = new ImageEffect('GD', 'image/gif');
        $this->gif = $gif_data;
    ...
    private function parse_frames()
    {
        ...
        while (self::GIF_BLOCK_END != ord($this->gif[$this->ptr])) {
            ...
            $this->ptr--;
            $this->frame_array[] = new GifFrame($lc_mod, $palette, $this->get_bytes($sum + 2), $head, $dimms, $gr_mod);
        }
    }
    ...
    private function pre_process_frame($img_data, $index)
    {
        ...
        @imagecopyresampled(
            $img_s,
            $str_img,
            $this->frame_array[$index]->pos_x,
            $this->frame_array[$index]->pos_y,
            0,
            0,
            $this->frame_array[$index]->width,
            $this->frame_array[$index]->height,
            $this->frame_array[$index]->width,
            $this->frame_array[$index]->height
        );
        ob_start();
